...
type PacketResult struct {
	...
	CallID string // Store the Call-ID from SIP header
}
...
// isEmpty checks if the Filter has any criteria specified
...
// Extract Call-ID from the packet if possible
func extractCallIDFromPacket(packet gopacket.Packet) string {
	...
	// Extract Call-ID from SIP headers
	headers := extractSIPHeaders(sip)
	if callID, ok := headers["Call-ID"]; ok {
		return callID
	}

	// Try alternate header format (some implementations use "i" as shorthand)
	if callID, ok := headers["i"]; ok {
		return callID
	}
...
// writePacketsToFilesBySplitCallID writes packets to multiple files, one per Call-ID
func writePacketsToFilesBySplitCallID(baseFilename string, packets []PacketResult) error {
	// Group packets by Call-ID
	callIDGroups := make(map[string][]PacketResult)
	unknownCallIDCount := 0

	for _, result := range packets {
		callID := result.CallID
		if callID == "" {
			// If we don't have a Call-ID, group under "unknown-N"
			callID = fmt.Sprintf("unknown-%d", unknownCallIDCount)
			unknownCallIDCount++
		}
		callIDGroups[callID] = append(callIDGroups[callID], result)
	}

	// Create a folder to put all the files in, based on timestamp
	folderName := fmt.Sprintf("calls_%d", time.Now().Unix())
	err := os.Mkdir(folderName, 0755)
	if err != nil {
		return fmt.Errorf("error creating output directory: %v", err)
	}

	// Sanitize Call-IDs for filenames
	sanitizeFilename := func(callID string) string {
	...
	// Create a file for each Call-ID
	totalFiles := 0
	for callID, packetsInCall := range callIDGroups {
		// Create a sanitized filename
		sanitizedCallID := sanitizeFilename(callID)
		// Use the base fileparts but insert the call ID
		baseExt := filepath.Ext(baseFilename)
		baseName := strings.TrimSuffix(filepath.Base(baseFilename), baseExt)

		// Create filename for this call
		outFilename := filepath.Join(folderName, fmt.Sprintf("%s_%s%s", baseName, sanitizedCallID, baseExt))

		// Write the packets for this call
		err := writePacketsToFile(outFilename, packetsInCall)
		if err != nil {
			return fmt.Errorf("error writing file for Call-ID %s: %v", callID, err)
		}

		logVerbose("Created file %s with %d packets for Call-ID: %s\n", outFilename, len(packetsInCall), callID)
		totalFiles++
	}

	fmt.Printf("Split %d packets into %d files by Call-ID in folder: %s\n", len(packets), totalFiles, folderName)
	return nil
}
...
// mergeChunkFilesGroupedByCallID merges multiple sorted PCAP chunk files
// into multiple output files grouped by Call-ID
func mergeChunkFilesGroupedByCallID(files []string, baseOutputFile string) (int, error) {
	if len(files) == 0 {
		return 0, nil
	}

	logVerbose("Merging %d chunk files grouped by Call-ID...\n", len(files))
	...
	// Process all chunk files to extract packets and group by Call-ID
	type packetWithCallID struct {
		callID    string
		ci        gopacket.CaptureInfo
		data      []byte
		timestamp time.Time
	...
	var allPackets []packetWithCallID

	// Open and read all files
	for fileIndex, file := range files {
		logVerbose("Reading chunk file %d: %s\n", fileIndex+1, file)

		// Open the file
		handle, err := openPcapFile(file)
		if err != nil {
			return 0, fmt.Errorf("error opening chunk file %s: %v", file, err)
		}
		...
		for packet := range packetSource.Packets() {
			...
			callID := extractCallIDFromPacket(packet)
			if callID == "" {
				callID = fmt.Sprintf("unknown-%d-%d", fileIndex, count)
			}

			// Store packet with its Call-ID
			allPackets = append(allPackets, packetWithCallID{
				callID:    callID,
				ci:        packet.Metadata().CaptureInfo,
				data:      packet.Data(),
				timestamp: packet.Metadata().Timestamp,
	...
	// Sort all packets by timestamp to maintain chronological order
	sort.Slice(allPackets, func(i, j int) bool {
		return allPackets[i].timestamp.Before(allPackets[j].timestamp)
	})

	// Group packets by Call-ID
	callIDGroups := make(map[string][]packetWithCallID)
	for _, p := range allPackets {
		callIDGroups[p.callID] = append(callIDGroups[p.callID], p)
	}
	...
	// Create a file for each Call-ID group
	baseExt := filepath.Ext(baseOutputFile)
	baseName := strings.TrimSuffix(filepath.Base(baseOutputFile), baseExt)

	totalFiles := 0
	totalPackets := 0

	for callID, packets := range callIDGroups {
		// Create a filename for this call
		sanitizedCallID := sanitizeFilename(callID)
		outFilename := filepath.Join(folderName, fmt.Sprintf("%s_%s%s", baseName, sanitizedCallID, baseExt))
		...
		// Write all packets for this call
		for _, p := range packets {
			if err := writer.WritePacket(p.ci, p.data); err != nil {
				f.Close()
				return 0, fmt.Errorf("error writing packet: %v", err)
			}
		}

		f.Close()
		logVerbose("Created file %s with %d packets for Call-ID: %s\n", outFilename, len(packets), callID)
		totalFiles++
		totalPackets += len(packets)
	}

	fmt.Printf("Split %d packets into %d files by Call-ID in folder: %s\n", totalPackets, totalFiles, folderName)
...
func processPCAPStreaming(filename string, filter Filter, outputFile string, numWorkers int, splitByCallID bool) (int, error) {
	var handle *pcap.Handle
	var err error
	...
	type matchResult struct {
		...
		data           []byte
		timestamp      time.Time
		originalPacket gopacket.Packet // Store original packet for Call-ID extraction
	}
	...
	if splitByCallID && !filter.isOnlyIPFilter() {
		...
		for _, match := range matchResults {
			callID := extractCallIDFromPacket(match.originalPacket)
			if callID == "" {
				callID = fmt.Sprintf("unknown-%d", unknownCallIDCount)
				unknownCallIDCount++
			}
			callIDGroups[callID] = append(callIDGroups[callID], match)
		}
		...
		// Write each call to a separate file
		totalFiles := 0
		for callID, matchesInCall := range callIDGroups {
			sanitizedCallID := sanitizeFilename(callID)
			baseExt := filepath.Ext(outputFile)
			baseName := strings.TrimSuffix(filepath.Base(outputFile), baseExt)
			...
			// Write all packets for this call
			for _, match := range matchesInCall {
			...
			f.Close()
			logVerbose("Created file %s with %d packets for Call-ID: %s\n", outFilename, len(matchesInCall), callID)
			totalFiles++
		}

		fmt.Printf("Split %d packets into %d files by Call-ID in folder: %s\n", len(matchResults), totalFiles, folderName)
...
// while maintaining packet order
func processPCAPChunked(filename string, filter Filter, outputFile string, numWorkers int, chunkSize int, splitByCallID bool) (int, error) {
	var handle *pcap.Handle
	var err error
	...
	if splitByCallID && !filter.isOnlyIPFilter() {
		...
		if len(tempFiles) > 0 {
			logVerbose("\nMerging %d chunk files into Call-ID grouped output...\n", len(tempFiles))
			matchCount, err = mergeChunkFilesGroupedByCallID(tempFiles, outputFile)
			if err != nil {
				return 0, fmt.Errorf("error merging chunk files by Call-ID: %v", err)
			}
			logVerbose("Merge complete. Output files written by Call-ID\n")
		} else {
			logVerbose("No matching packets found in any chunk\n")
		}
	} else {
		// Standard merge to a single file
		if len(tempFiles) > 0 {
...
func main() {
	...
	splitByCall := fs.Bool("split", false, "Split output into separate files by Call-ID")
	...
	if *chunkMode {
		...
		matchCount, err = processPCAPChunked(pcapFile, filter, tempPCAP, numWorkers, *chunkSizeFlag, *splitByCall)
	} else if *streamOutput {
		// Stream output directly to file (memory efficient)
		matchCount, err = processPCAPStreaming(pcapFile, filter, tempPCAP, numWorkers, *splitByCall)
	} else {
